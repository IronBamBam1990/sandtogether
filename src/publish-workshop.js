...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.73-beta — FIXES the "client cannot dig / client mines on his own world" family of reports (J.Slayer, Akriz, derErste67, ZeroHazard). Root cause found and reproduced end-to-end in a two-instance test: loading the world the host sends RELOADS the game page on the joining player. The mod restarts from scratch, but the network connection lives in another process, so the host never learns it should re-send the world — the joining player ends up standing in the host world with a DEAD mirror, playing locally, and nothing they dig ever reaches the host. The client now asks the host for the world stream itself and the mirror recovers within seconds (verified). Reminder: if you installed between Aug 18 and Aug 20, run install.bat ONCE first — the auto-updater itself was broken in those versions, so updates cannot reach you until you do.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
