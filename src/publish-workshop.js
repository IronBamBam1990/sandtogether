...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.138-beta - THREE FIXES FOR THINGS THAT LOOKED LIKE BROKEN WORLD DATA. Red bars and blocks that no one could remove often were not in the world at all: they were the preview of what your teammate was about to place, drawn in their colour and never cleared, because that preview is only cleared by their next position packet and position packets stop when a player stands still. It now expires two seconds after their last build intent. Second, the client was silently missing structures - measured 1019 on the host against 841 on the client - because the client sliced snapshot work and deferred the remainder to the next snapshot, while the host skips sending one when nothing changed; the client now finishes the deferred work itself, which brought it back to 1019 with no other change. Third, a foundation tile left behind with no structure on it is real garbage that renders red and that the game will not clean up: after a deletion there were twelve of them at identical coordinates on both machines. Host and single player now sweep around the players every five seconds and remove tiles confirmed orphaned across two passes six seconds apart, so a tile merely queued for placement is never taken. Also in this build: unlocked buildings and items travel from the host so research by either player shows for both, and the panel explains why connecting to your own public address from inside your own network fails (most routers refuse it - use the local address when you are on the same network).',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
