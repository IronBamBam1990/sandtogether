...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.145-beta - THE STUTTER OVER THE INTERNET IS FIXED, and it was our bug, not your connection. Sockets were left with Nagle\'s algorithm on, which holds a small write until the previous one is acknowledged - only one small packet may be in flight at a time. Player positions go out 30 times a second, so on an 80 ms link roughly twelve of them got through per second and the rest arrived in bursts; worse, your own actions - digging, grabbing, placing - queued in the same TCP stream behind them. That is why sessions felt rubbery and a teammate sometimes could not pick anything up, even with a fast connection, a strong PC and a good ping. On a LAN the round trip is under a millisecond, so nothing showed there, which is how this survived so long. Both ends now disable it. If you played over the internet and blamed your line, try again after this update. Reported by friberg, whose 80 ms ping and 600/300 connection made it clear the link was not the problem.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
