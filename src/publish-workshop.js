...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.157-beta - HOTFIX for 0.9.156, update immediately: the full-tank blow-away could DELETE the material instead of blowing it. The removal inside the blow step went through a deferred queue, so the re-create fired on a still-occupied cell and silently failed - then the late removal erased the element. Remove and re-create now happen synchronously in one step. Caught by the author within minutes of testing 0.9.156 on a level-6 world; verified after the fix with a fixed measuring region including type conversions (sand turning to wet sand in water): zero net material loss, pellets visibly blown from the nozzle. If you grabbed 0.9.156 in the last hours, this is the one to be on.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
