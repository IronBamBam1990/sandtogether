...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.60 - v0.9.63 — CRITICAL FIXES for 0.9.59 lobby problems (reports: Akriz, derErste67, TCentraL): (1) INSTANT KICK fixed — joining a host who was still in the main menu disconnected you within a second; a host in the menu no longer streams anything and the world is sent automatically the moment the host loads in. (2) CORRUPTED MAP on the joining side fixed (yellow garbage, digging seemingly dead) — two world transfers could interleave into one download; transfers now carry ids. (3) Session status you can SEE: colored role badge (OFFLINE / HOSTING / CONNECTED), live player list, contextual buttons, join/leave chat notes; the collapsed panel is now a tiny "ST" pill. (4) Set your NICK in the lobby (LAN players were all "Player"); save picker next to "Load last save & PLAY". (5) Big-map progress: chunk countdown + "loading the host world..." status. (6) Linux installer heals Windows line endings (confirmed working by PsychoSpark). Auto-updates at next launch.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
