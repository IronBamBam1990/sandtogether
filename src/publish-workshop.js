...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.147-beta - amends 0.9.146 (released earlier today), update straight to this one. The 0.9.146 filter fix applied the joining player\'s filter config to every structure that carries a filter - but the gloom emitter and the critter fence carry FIXED filters the game assigns itself, and stamping a player config over them would break how they work. The fix now targets only structures the game itself marks as filter-configurable (filters, advanced filters, filter walls), and copy-paste is handled separately: a pasted structure\'s filter is applied verbatim to the same structure type, which also covers machines that get their filter only through copying. Forced behaviour is preserved - pieces that must act on liquids and gases keep those flags regardless of the placing player\'s selection. If you already grabbed 0.9.146: nothing broke unless a joining player placed a gloom emitter today; this release just closes that hole.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
