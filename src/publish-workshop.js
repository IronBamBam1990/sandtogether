...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.144-beta - TWO THINGS, both from player reports. (1) Steam relay: when a session runs through Valve\'s relay and the round trip climbs into whole seconds, the mod now says so out loud instead of leaving you to guess why the world stopped moving. The relay throttles bandwidth and your own actions queue up behind the world transfer, so the fix is to host directly: the warning tells the host to restart as Host (Internet - direct) and the other player to join by address. (2) The transport label on the joining side said LAN for every join-by-address session, including connections across the internet, which read as if the mod were switching people onto a local network (report: Shadow City Empire). It now reads Internet for a public address and LAN only for a local or VPN one. The Workshop page also gained a short port-forwarding checklist: the mod listens on TCP 27777, and the host\'s Windows Firewall must allow Sandustry.exe on public networks.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
