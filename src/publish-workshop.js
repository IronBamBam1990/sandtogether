...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.153-beta - BIG-FACTORY BANDWIDTH CUT 10x (report: Sessional - \'horrible lag with a boatload of materials on conveyors\'). Items moving on conveyors change the world every tick, so a belt-heavy area kept re-sending itself about ten times a second even when no player was anywhere near it - measured 5 MB/s of mirror traffic on an 84,000-structure factory with both players away from the base. Distant areas now refresh at most ~1.6 times a second while everything within about two screens of any player keeps the full rate - so what you are looking at is as smooth as before, and what nobody watches stops eating the host\'s upload (same scenario now: 0.5 MB/s -> fits home connections). One-off changes far away (a building placed, a tunnel dug) still arrive immediately. Nothing is skipped, only rescheduled. Update both sides.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
