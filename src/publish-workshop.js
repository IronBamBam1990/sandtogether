...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.152-beta - THE FROZEN-WORLD-ON-JOIN CASE THAT SURVIVED 0.9.150 IS CLOSED (report: Quadbro - static world on the joining side while the host received every action). Root cause: loading the transferred world reloads the joining player\'s game, wiping the mod\'s trust state - and importing a world you already had a copy of assigns it a NEW local world id, so every mirror packet failed the id check forever after. The world-session token introduced in 0.9.150 now rides in every mirror packet and survives the reload, so a player who loaded exactly the host\'s transfer is trusted regardless of local ids - and the host no longer re-sends the world on every handshake when the token already matches. Verified machine-to-machine by forcing a foreign world id on a connected client: the mirror kept flowing. Update both sides.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
