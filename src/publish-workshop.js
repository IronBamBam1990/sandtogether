...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.126-beta - THE CLIENT NO LONGER FREEZES, AND THE GRABBER FINALLY MATCHES THE HOST. Freeze root cause: when the client renderer restarted (which the mod itself triggers when auto-loading the host world), its ack counter went back to zero while the host was already at packet 2651. The host read that as a client 26 batches behind, paused sending, and the client could never ack because nothing arrived - both sides waited forever. Hello and resync now reset the ack baseline and the host self-heals after 8 seconds without ack progress. World packets travel as binary frames instead of base64 inside JSON on LAN and direct connections: same data, 25 percent fewer bytes, no encode or decode work on either side. Incoming world data is applied in slices across frames, so a big packet no longer blocks the picture - worst client frame went from 237 ms to 16 ms while throughput rose to several MB/s. Grabber: the game allocates the tank matrix at maximum size and reports the active slots through data.size, but the bridge read the array length, so the client harvested at allocation size no matter what you had set and stored material outside the active window where the game cannot see it - material appeared to vanish. Collecting now continues while you hold the button, the tank header follows the game invariant, merged particles resolve to their real material, and anything that does not fit goes back on the map instead of being destroyed.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
