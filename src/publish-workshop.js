...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.154-beta - FPS STUTTER HUNT, ROUND ONE. Reports of frame drops in co-op had a measurable core: every time the set of buildings changed, the host serialized the entire structure list in one frame and the joining player applied it in one frame - on an 84,000-structure factory that was a ~140 ms hitch, several times a minute while anyone was building. Structure snapshots are now SLICED: the host serializes one 4,000-structure part per frame, the client applies parts as they arrive and reconciles with a 4 ms per-frame budget. Measured on the same factory: worst client hitch down from 140 ms to 19 ms. Also new: a permanent frame profiler - when the game stutters in a session, the log now says how many frames went over budget and whose cost it was, so your next report pinpoints the culprit for me. This round follows 0.9.153\'s 10x bandwidth cut; more rounds will come as profiler data arrives. Update both sides.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
