...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.160-beta - installer fix for Steam\\u2019s re-minified builds (report: cayden.sieteski). Steam can ship differently-minified game code under the SAME version number - on such builds every patch anchor missed and the installer wrongly claimed "version not supported". The installer now detects the module alias straight from your game\\u2019s code and adapts the hooks automatically; verified on a simulated re-minified build with all hooks applying, including the critical one. If you hit the "NOT supported" error on 0.5.5, update the mod and run the installer again. If some non-critical hooks still print a skip warning on your build, send us your unpatched bundle.js and we will port them properly. (Previous release 0.9.159:) the big Corraller/client round plus three community reports fixed. CORRALLER on the client is now 1:1 with the host: critter duplication is gone (captures were counted twice - local finalize + host finalize; the client no longer finalizes, the host ignores the legacy collect for corralled critters), the permanent screen brightening is fixed (entity sync replaced critter objects and orphaned their eternal lights; objects now keep identity so lights are extinguished correctly), releases show the muzzle launch instantly (local echo with full physics, handover to the host critter in 600 ms), flying critters arc and bounce off walls (host velocities + local physics, position corrected only on drift), capture completion plays sound/flash/particles, the bucket counter updates live, a sprite janitor removes any stuck critter graphics, and capture requests are deduplicated (one per critter instead of 60/s). REPORTS: MaxMasterB - host filters no longer get silently reverted by the client config scan (filter rides along only when the filter itself changed), and the upgrade orb no longer locks the player who did not take it (pendingChoice accepted only on a new orb; choices are merged, not overwritten - the first picker also cannot wipe the other player\\u2019s open popup or nodes). darkalien - planter boxes placed by the client pass gold again (the placer\\u2019s generic default filter no longer overwrites structure-specific filters like the Planter Box gold/seed pass), and story unlocks (teleporter, anomaly/Void destination) now activate live on the client without a restart (new story steps from the team are emitted locally, like the tech sync). Also: volcanizer/caulk/flamethrower create elements through the when-idle queue with proper velocity and duration.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
