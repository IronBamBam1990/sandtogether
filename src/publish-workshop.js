...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.139-beta - RED LINES LEFT BEHIND WHEN YOU DELETE A CONVEYOR ARE GONE. Blocks deleted fine, but a thin red line sometimes stayed where a belt had been. Those are orphaned terrain tiles: a conveyor lays down its own terrain type, and the cleanup shipped in the previous build only swept foundation tiles. Measured in a live world - and identical on host and client, so the garbage was genuinely in the world - 132 orphaned ConveyorLeft tiles and 36 ConveyorRight tiles sitting with no structure on them; a row of those is exactly the red line you saw. The sweep now covers every terrain type that only ever comes from a structure (foundations, sliding blocks, conveyors, shakers, velocity soakers, growers) and still never touches natural ground such as stone or ice, and it confirms a tile across two passes six seconds apart so a tile merely queued for placement is never removed. Also in this build: the client no longer silently misses structures, and a teammate build preview no longer stays frozen on your screen after they stop moving.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
