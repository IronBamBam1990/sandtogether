...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.146-beta - FILTERS PLACED BY A JOINING PLAYER NOW KEEP THAT PLAYER\'S CONFIG (reports: Maelle, Moonbugy). The game stamps a freshly placed filter with the filter configuration of whoever places it - and in a session the host places everything on the joining player\'s behalf, so every filter a client put down came out with the host\'s current selection, no matter what the client had configured or copy-pasted. The joining player now sends their own filter configuration with the placement, and the host applies it to the built structure before mirroring it out - including copy-paste, which previously lost the filter because the copied structure lives only in the copying player\'s session. Pass-through pieces (filter walls, pass-through conveyors) keep the flags the game forces on them. Changing the filter of an ALREADY placed structure was fixed back in 0.9.142 and still works; this closes the placement half.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
