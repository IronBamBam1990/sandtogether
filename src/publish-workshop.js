...
[b]Author: Kamil Padula[/b] — [b]Contributors: dotNine, Knight-HD, DwoaC, Cr0ss0vr, TCentraL, AlyxiaFox, NanYu_sad., darkalien, Ylvakiller, Qustux, UwUDev[/b]
...
[h2]🎮 Game versions[/h2]
[b]v0.9.166+[/b] = game [b]0.5.6[/b]. Older game (0.5.2-0.5.5) still works, minus the newest extras. All players: same game + mod version.
...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'IMPORTANT FIX - a joining player could DELETE the host\u2019s buildings. If a friend joined your world, anything sharing a cell with a pipe could be destroyed on your side: walls the pipes run through, pumps, liquid vents. This was a real, permanent loss in the host\u2019s world, not a display glitch, and it is why some of you saw parts of your factory vanish the moment someone connected. Cause: the client reports its own local structure removals to the host as demolitions, and two background workers added in 0.9.154 (finishing deferred snapshot work and cleaning up ghost structures) were missing the guard that says "this is the mod applying the host\u2019s world, not the player demolishing something". While a joining client rebuilt your world, every building the game itself removed as a conflict was reported back to you as a genuine demolition. Measured on a large test base across a join: 164 buildings destroyed before the fix (all 11 pumps and 93 of 94 vents), 0 after - the world now comes through a join completely untouched. Also hardened: leftover-rubble cleanup now knows about pipes, so walls carrying pipes are never mistaken for debris, and ghost cleanup no longer removes a pipe by deleting whatever building stands on its cell. Please update on BOTH machines before playing together again. Still on the list: the joining player\u2019s own view can be missing those pipe-cell buildings until they reconnect - nothing is lost, your world is intact.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
