...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.161-beta - full line-by-line audit of the mod against the official Sandkit docs; five real defects fixed. Biggest one: the mod\\u2019s per-frame hook now wears armor - an uncaught exception inside the game\\u2019s frame pipeline kills the game\\u2019s promise-chained frame loop PERMANENTLY (static screen while the world sync keeps running underneath); every error is now caught, logged with a stack and swallowed, and a renderer-wide error recorder keeps the last uncaught rejection in memory, so any future freeze names its culprit in seconds. Also: joining clients now reliably receive the host\\u2019s teleport zones and unlocked-item list (both used to be sent only on change and the one send after world load raced the client\\u2019s mirror start - a fresh client kept its own locally generated zones forever; verified converged), a mangled IP regex from 0.9.133 (invisible control characters) was restored so the "use the host\\u2019s local address" hint works again, a session-reset statement that had been swallowed by a comment no longer leaks stale delivery records between host sessions, and shake residue is created through the when-idle queue like lava/ice so it cannot lose the race against the simulation worker. (Previous: 0.9.160 installer adaptation for re-minified builds, 0.9.159 Corraller round.)',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
