...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.142-beta - THREE WORKSHOP REPORTS FIXED. (1) "Error: net.HostDirect is not a function" when hosting direct: the auto-updater only added the network bridge to the game when it was missing and never replaced an old one, so anyone who installed before 0.9.79 kept a bridge without hostDirect. It now swaps the bridge like the installer does - one more launch and it is current. (2) "I cannot destroy the world with the Laser when I join my friend": every dig carries a profile (fromDrill, fromRocketExplosion...) that decides which material tier it can break; the joining player sent digs without it, so the host replayed the laser as a shovel. The profile now travels with the dig; laser energy is charged on the host (no power on the host = no hole) and the handheld drill, which digs through a path the mirror discards, is forwarded too. (3) "Filters only work when host configures them": a filter setting lives in structure.filter, not in data, and only data was synchronised. Filters, shakers and growers now sync both ways - set by the joining player or by the host. All three verified live on two instances.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
