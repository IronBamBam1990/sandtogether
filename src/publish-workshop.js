...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.132-beta - CLIENT TOOLS AND PROGRESSION ARE FIXED. Joining a host no longer leaves you with the tools, research and buildings of your previous game: the client received the host save but a guard skipped loading it, so the mirror painted the host terrain into a different world and everything looked right while your player state stayed local (measured on a fresh world: host 4 items, client 21). The client now loads the host world whenever its world id differs. Tools also worked once and then died - weapon cooldowns are game-time stamps and the client inherited stamps from another session, half an hour in the future, so the cooldown never expired again and digging silently did nothing; those stamps are straightened out now and the local profile no longer restores your inventory at all. Half the screen turning yellow with the shovel out is gone as well: the mod used to stop the client with the manager pause flag, which also gates work the renderer needs every frame, so the client now brakes with simulation speed instead - and that removes the desync after an automatic save for free, because saving toggles the pause flag while nothing touches the speed. Grabber shaking works on the client again, and the grabber harvests exactly the grid size you set (the game reports it through data.size, which the bridge was reading wrong).',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
