...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.135-beta - SHARED PROGRESSION, AND A FIX FOR A JOIN LOOP INTRODUCED IN 0.9.132. Unlocked buildings and items now travel from the host in the state packet: a joining client adds what it is missing and drops what the host does not have, without touching items it already holds, so research done by either player shows up for both. Verified live: a client sitting at 2 buildings and no research pulled up to the host state within seconds. The previous build tried to force the client into the host world by matching world ids, but loading a transferred save assigns a NEW world id, so the check never settled and the client reloaded over and over until it fell out of the session - the rescue load is now a single attempt per session. Direct internet hosting was verified end to end from a machine outside the network: UPnP opened the port, the handshake completed and the host streamed a full save transfer plus 969 mirror packets, 11.37 MB in 20 seconds. One warning worth knowing: from inside your own network you cannot connect to your own public address - most routers refuse it - so when both players are on the same network, use the local 192.168.x.x address. The panel now says this instead of leaving you guessing. Also fixed since 0.9.126: tools that stopped working after one use (weapon cooldowns are game-time stamps and the client inherited stamps from another session, so digging silently did nothing), half the screen turning yellow with the shovel out, desync after an automatic save, and grabber shaking on the client. Thanks to Cr0ss0vr, whose report and PR kept the progression thread alive until the real cause surfaced.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
