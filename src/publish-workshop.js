...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.143-beta - TWO FIXES. (1) Big factories: 0.9.142 made the joining player freeze for about 2 seconds (worst 5) on every structure snapshot in a 90,000-structure world and then drop - the client was calling the game\'s per-structure update for thousands of changes, and that update scans the whole structure list each time. It now only does that when a tile mode really changed; data and filter are updated by plain assignment. If you saw freezes or disconnects on 0.9.142, this is it. (2) Conveyors placed over blocks (report: hunters01): a structure built on top of terrain is queued - it sits on the blocks and nothing is carved out. The joining player\'s placement lost that information, so the host built a full conveyor and deleted the blocks, stone included. The client now sends the clearance it validated with, the host builds with it, and queued/frame flags travel in the structure packets, so a conveyor the host lays over blocks is queued on the client too. Verified live: client-placed Conveyor Mk.2 inside stone stays queued on the host with the stone intact.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
