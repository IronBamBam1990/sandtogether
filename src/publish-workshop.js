...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.164-beta - GRABBER DUPE FIX (report: Maelle). When a client used the grabber, the host removed the element and immediately credited its type to the client tank WITHOUT checking the removal actually took effect - so in some cases material could stay in the world AND land in the tank. The host now confirms an element really left the world before giving it to you, and blocks a cell whose removal failed for a moment. Normal grabbing is unchanged (measured). ALSO INVESTIGATED: the Recall Shard reporting "gloom missing at the recall portal" for clients. I could NOT reproduce it on a host+client pair - the portal position, its built state and the gloom cells all mirror to the client exactly, and running the game own check on both sides agreed. Note the game requires ALL 16 cells (4x4) directly above the portal base to be gloom, so one different cell gives that message. If it happens to you, please post your client log (Windows: %APPDATA%\\Sandustry\\logs\\main.log) and ideally the save - that will let me pin it down. PREVIOUS: INSTALLER FIXES for macOS and Linux. (1) The installer used to print its messages - including fatal errors - in POLISH, so players outside Poland could not read what went wrong (report: thatsmaik). Everything is English now, and a failed install prints what to do next. (2) If your macOS/Linux install is stuck on an older mod version with a "not supported" style error: that was the alias-detection bug fixed in v0.9.163 - on an already-patched 0.5.5 game the old installer could not detect the game build and aborted. Launch the game once through Steam so it downloads this Workshop version, then run install.command (macOS) / bash install-linux.sh (Linux) again. If it still fails: Steam -> Sandustry -> Properties -> Installed Files -> Verify integrity, delete the unpacked app folder, run the installer again. (3) LINUX INSTALLER FIX (thanks UwUDev, PR #19) - install-linux.sh picked the first executable file in the game folder as the game binary, so on setups where LICENSE.electron.txt carries the executable bit the installer tried to run the LICENSE FILE and died with "syntax error near unexpected token". It now checks for ELF magic bytes instead of trusting the executable bit. Linux players who hit this: re-run bash install-linux.sh from this version. Mod runtime unchanged.\\n\\nv0.9.163-beta - FULL SUPPORT FOR THE NEW GAME BUILD 0.5.5. The 0.5.5 update shipped genuinely new game code (new dig/pattern system, reworked internals, re-minified bundle) and 25 of our 29 code anchors stopped matching - the mod half-installed: multiplayer connected and the world synced, but the shovel did not dig, client-placed buildings were invisible to the host, sand fell through client shakers, and the volcanizer, grabber shake, cryoblaster, flamethrower, vacuum, demolisher, caulk blaster and corraller did not sync (reports: The_Dark_LP, DW|Joergschen - thank you!). All 25 anchors are re-ported to the 0.5.5 build and verified live: client digging, building placement, grabber shake residue, volcanizer lava with velocity, corraller capture/release and the rest replay correctly on the host again. The installer also detects the module alias on already-patched bundles now, and prints a clear red warning when many hooks miss (instead of a soft note), so a future game update will be obvious at install time. IF YOUR TOOLS DO NOT WORK IN CO-OP: update the game to 0.5.5, get this mod version and re-run install.bat - all players must be on the same game and mod version.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
