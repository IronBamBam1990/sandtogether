...
[h2]🎮 Game versions[/h2]
[b]v0.9.165+[/b] = game [b]0.5.6[/b]. Older game (0.5.2-0.5.5) still works, minus the newest extras. All players: same game + mod version.
...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'SUPPORT FOR GAME 0.5.6 - if your tools stopped syncing, this is the fix. The 0.5.6 update re-minified the game code and 21 of our 29 hooks stopped matching, so the installer printed \"21 features did not match\" and co-op ran half broken: it connected, the world streamed, but digging, building, the grabber and most tools silently did nothing for the other player. All anchors are re-ported and verified against the clean 0.5.6 game code (32/32); older builds 0.5.2-0.5.5 keep working through the existing chain. Huge thanks to Qustux, who tracked this down and sent the whole fix as a pull request, together with: a fix for the join hang (world transfer could restart over and over), smoother mirroring on fast connections, Ctrl+Z that is now per player instead of shared, correctly sized overlays, and a brand new ACTION PREVIEW - you now see what the other player is holding and where they are about to build, dig or grab, with real structure graphics and item names. Reviewed and tested on a two-instance session before release: one change was dropped because it forced a full structure snapshot every ~2 seconds on worlds with unfinished blueprints, which stopped structures from syncing. UPDATE THE GAME TO 0.5.6 AND RE-RUN install.bat - all players need the same game and mod version.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
